...
for i in range(0, len(dataset)):
    if(dataset['Description'][i]==9149):
       dataset['Description'][i] = str(9149)
...
cv = CountVectorizer(max_features=26) # train data keyword size(incwords size)
...
lclassifier = LogisticRegression(random_state = 0,multi_class='ovr')
lclassifier.fit(X, y)
